...
export function InvoiceForm(props: InvoiceFormProps) {
  ...
  const { form, isLoading, error, onSubmit } = useInvoiceForm({
  ...
  useEffect(() => {
    watch('items').forEach((_: any, idx: number) =>
      setValue(`items.${idx}.tax_rate`, Number(tax_rate)),
    );
  }, [tax_rate]);
  ...
  const subtotal = useMemo(
    () =>
      items.reduce(
        (sum: number, item: InvoiceItem) => sum + Number(item.quantity) * Number(item.unit_price),
        0,
      ),
    [items],
  );
