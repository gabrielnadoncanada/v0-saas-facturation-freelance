...
export async function createSubtask(
...
): Promise<{ subtask: Task; projectId: string }> {
  ...
  const task = await fetchById<{ project_id: string }>(supabase, 'tasks', taskId, 'project_id');

  // Vérifier que le projet appartient à l'organisation
  const projects = await fetchList(supabase, 'projects', 'id', {
    id: task.project_id,
  ...
  // Créer la sous-tâche
  const subtask = await insertRecord<Task>(supabase, 'tasks', {
    ...formData,
