...
export function TaskGantt({ tasks, projectStartDate }: { tasks: Task[]; projectStartDate?: string }) {
  ...
  const scrollToToday = () => {
    ...
    const offset = differenceInCalendarDays(today, start) * dayWidth - (containerRef.current?.clientWidth || 0) / 2
    ...
    <div>
      <div className="flex items-center justify-between mb-2">
        <div className="space-x-2">
          <button className="text-xs px-2 py-1 border rounded" onClick={() => setZoom("days")}>Jour</button>
          <button className="text-xs px-2 py-1 border rounded" onClick={() => setZoom("weeks")}>Semaine</button>
          <button className="text-xs px-2 py-1 border rounded" onClick={() => setZoom("months")}>Mois</button>
          <button className="text-xs px-2 py-1 border rounded" onClick={() => setZoom("quarters")}>Trimestre</button>
        </div>
        <button className="text-xs px-2 py-1 border rounded" onClick={scrollToToday}>Aujourd'hui</button>
      </div>
        ...
        <div style={{ width: totalWidth }} className="space-y-2">
          <div className="relative h-6 flex text-xs">
            {ticks.map((date, idx) => {
              ...
              if (zoom === "quarters") label = `T${Math.floor(date.getMonth() / 3) + 1} ${date.getFullYear()}`
              const width =
                zoom === "days"
                  ? dayWidth
                  : zoom === "weeks"
